...
class JtUnfolder(object):
    def __init__(self, name, **kwargs):
        ...
        if self._jetBinBorders is not None:
            self._jetPtBins = [
                (a, b) for a, b in zip(self._jetBinBorders, self._jetBinBorders[1:])
            ]
    ...
    def createToyTraining(self, rootFile, numberEvents):
        ...
        self._hJetPtIn = self._f.Get(
            "AliJJetJtTask/AliJJetJtHistManager/JetPt/JetPtNFin{:02d}".format(2)
        )
        ...
        self._hJtTestMeas2D = Hist2D(
            self._LogBinsJt, self._jetBinBorders
        )  # Needs a better name
        ...
        self._responseJetPtCoarse = RooUnfoldResponse(
            self._hJetPtMeasCoarse, self._hJetPtTrueCoarse
        )
    ...
    def unfold(self):
        ...
        if not self._fillFakes:
            print(
                "Scale hJtFake2D by {}".format(
                    1.0 * sum(self._numberJetsMeasBin) / self._numberJetsMeasTrain
                )
            )
            self._hJtFake2D.Scale(
                1.0 * sum(self._numberJetsMeasBin) / self._numberJetsMeasTrain
            )
        ...
        self._hJetPtRecoCoarse = unfoldJetPt(
            self._hJetPtMeasCoarse, self._responseJetPtCoarse, self._jetBinBorders
        )
        self._hJetPtReco = unfoldJetPt(
            self._hJetPtMeas, self._responseJetPt, self._LogBinsX
        )

        self._numberJetsMeasFromHist = [
            self._hJetPtMeasCoarse.GetBinContent(i)
            for i in range(1, self._hJetPtMeasCoarse.GetNbinsX() + 1)
        ]
        if not self._IsData:
            self._numberJetsTrueFromHist = [
                self._hJetPtTrueCoarse.GetBinContent(i)
                for i in range(1, self._hJetPtTrueCoarse.GetNbinsX() + 1)
            ]
        ...
        if not self._IsData:
            ...
            self._hJtTestTrueBin = [
                makeHist(
                    self._hJtTestTrue2D.ProjectionX("histMeas{}".format(i), i, i),
                    bins=self._LogBinsJt,
                )
                for i in range(1, len(self._jetBinBorders))
            ]
    ...
    def plotResponse(self):
        ...
        for ax, h in zip(axs, (self._responseJetPt, self._response2D)):
            ...
            xbins = [
                ...
                for iBin in range(1, hResponse.GetNbinsX() + 2)
            ]
            ybins = [
                hResponse.GetYaxis().GetBinLowEdge(iBin)
                for iBin in range(1, hResponse.GetNbinsY() + 2)
            ]
